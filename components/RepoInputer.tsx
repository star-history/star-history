...
export default function RepoInputer() {
  ...
  return (
    <div className="w-full px-3 shrink-0 flex flex-col justify-start items-center px-3 shrink-0 flex flex-col justify-start items-center">
      <div
        className={`w-auto mx-auto mt-6 mb-2 flex flex-row justify-center items-center flex-wrap ${
          state.latestBlog ? '' : 'invisible'
        }`}
      >
        <span className="px-2 -mt-px leading-7 rounded mr-2 text-sm bg-green-100 text-green-600 font-medium">
          What's new
        </span>
        <a className="text-gray-700 hover:underline" href={`/blog/${state.latestBlog?.slug}`}>
      ...
      <div className="w-auto sm:w-full grow max-w-3xl 2xl:max-w-4xl mt-4 flex flex-row justify-center items-center shadow-inner border border-solid border-black rounded">
        ...
        />
        <button
          className={`h-9 pl-4 pr-4 whitespace-nowrap w-auto text-black border-l border-black hover:text-light ${
      ...
      <div className="w-full mt-4 flex flex-row justify-center items-center">
        <div
          className={`w-full max-w-2xl flex flex-row flex-wrap justify-center items-center ${
            state.repos.length > 0 ? '' : 'invisible'
          }`}
        >
          {state.repos.map((item) => (
            <div
              key={item.name}
              className="leading-8 px-3 pr-2 mb-2 text-dark rounded flex flex-row justify-center items-center border mr-3 last:mr-0"
            >
              <span
                className="relative w-3 h-3 mr-1 flex flex-row justify-center items-center cursor-pointer hover:opacity-60"
                onClick={() => handleDeleteRepoBtnClick(item.name)}
              >
                <span className="w-3 rotate-45 h-px bg-black absolute top-1/2"></span>
                <span className="w-3 -rotate-45 h-px bg-black absolute top-1/2"></span>
              </span>
              <span
                className={`mr-1 cursor-pointer hover:line-through select-none ${
                  item.visible ? '' : 'line-through text-gray-400'
                }`}
          ...
          <button
            className="leading-8 mb-2 text-black hover:bg-gray-100 px-3 rounded border border-transparent"
            onClick={handleClearAllRepoBtnClick}
          >
            Clear all
          </button>
        </div>
