...
export default function TokenSettingDialog({ onClose }: TokenSettingDialogProps) {
  const store = useAppStore();
  const [token, setToken] = useState(store.token);

  const tokenCache = store.token;
  ...
  return (
    <Dialog>
      <div className="max-w-2xl justify-start items-start bg-white rounded-md">
        ...
        <main className="w-full flex flex-col justify-start items-start p-4 pr-5 text-base">
          <p>
            Star-history uses GitHub API to retrieve repository metadata. You may see this page
            because you have hit the{' '}
            <a
        ...
        <footer className="w-full flex flex-row justify-end bg-gray-100 items-center p-4 pr-5 border-t rounded-b-md">
          <button
            className="pl-4 pr-4 h-10 rounded-md bg-green-500 shadow-inner text-light text-base hover:bg-green-600"
