...
interface TokenSettingDialogProps {
  onClose: () => void;
  tokenCache: boolean; // or whatever the type of tokenCache is supposed to be
}
export default function TokenSettingDialog({ onClose, tokenCache }: TokenSettingDialogProps) {
  ...
  const handleSaveTokenBtnClick = () => {
    console.log("Token before saving:", token); // Check the token value before saving
    store.setToken(token);
    console.log("Token in store after saving:", store.token); // Check the token value in the store after saving
    storage.set({
      accessTokenCache: token,
    });
    console.log("Token in local storage after saving:", localStorage.getItem("accessTokenCache")); // Check the token value in local storage after saving
  };
    ...
    <Dialog>
      <div className="max-w-2xl justify-start items-start bg-white rounded-md overflow-hidden" >
        <header className="w-full flex flex-row justify-between items-center p-4 pr-5 bg-gray-100 rounded-t-lg">
          <span className="text-2xl">{tokenCache ? 'Edit' : 'Add'} GitHub Access Token</span>
          <FaTimesCircle className="fas fa-times-circle text-xl text-gray-400 cursor-pointer hover:text-gray-500"
        ...
        <main className="w-full flex flex-col justify-start items-start p-4 pr-5 text-base">
          ...
          see this page because you have hit the <a
        ...
        <p>
          ...
          >to unlimit it. If you don't already have one, <a
            className="text-blue-500"
